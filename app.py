...
try:
    ...
    MODEL = None
    for model in available_models:
        if "gemini-2.0-flash" in model.name.lower():  # Convertir a minúsculas para coincidencia insensible a mayúsculas
            MODEL = model.name
            break

    if MODEL is None:
        st.error("No se encontró un modelo 'gemini-2.0-flash' (o similar) en la lista de modelos disponibles. Revisa la salida de ListModels().")
        st.stop()
